...
The mechanism underneath all of this is standard Beamer, nothing specific to this group's template: `\includegraphics` is itself overlay-aware, so `\includegraphics<M>[page=N]{file.pdf}` pulls page `N` out of a multi-page PDF and shows it only on overlay `M` — no `\only` wrapper needed. Beamer's overlay specification (`<M>`) supports single overlays, ranges, and open-ended ranges natively, so the same three shapes as [01 — Artboards](01-Artboards.md#never-rescale-a-figure-once-its-placed) always work. This is real, already-used syntax in the reference talk's own appendix (`Coupling space and detuning flexibility`), which drives two different figures directly off it:

```latex
\includegraphics<1>[height=0.85\textheight]{./Figures/Figure_SupMat_Coupling.pdf}
\includegraphics<2>[width=\textwidth]{./Figures/Figure_SupMat_DifferentGeometry.pdf}
```

The same pattern drives a multi-page animation, one call per page:

```latex
\includegraphics<1>[page=1]{./Figures/WaferScale.pdf}
\includegraphics<2>[page=2]{./Figures/WaferScale.pdf}
\includegraphics<3->[page=3]{./Figures/WaferScale.pdf}
```

This works in any Beamer document — no theme, no package, no macro required. Its downside is purely readability: the filename and `\includegraphics` are retyped on every line, and a mapping with several pages and overlay ranges turns into a wall of near-identical lines that's easy to miscount while editing.

## The shortcut: `\imageseq` (this template only)

`JqiNanoBeamerTemplate` ships `\imageseq[opts]{file.pdf}{mapping}` to collapse exactly that repetition into one line — it is **not** a Beamer built-in, it only exists if your talk is built on this group's template, but if it is, it's the version to reach for: the filename is written once, and the `page:overlay` pairs read like a table instead of a stack of duplicated commands.

```latex
\imageseq{./Figures/WaferScale.pdf}{1:1,2:2,3:3}
```

is the same three-step reveal as the three `\includegraphics<M>[page=N]` lines above, with the filename written once instead of three times. The mapping grammar: `page:overlay`, where `overlay` is a single number, a range (`3-5`), or open-ended (`6-`) — the exact same overlay shapes `<M>` accepts directly, just written once per page instead of once per line.
...
% plain \includegraphics<M>[page=N] this would be four repeated lines,
% filename included each time; \imageseq keeps it to one.
